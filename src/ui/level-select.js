export class LevelSelectScreen {
  ...
  show(maxClearedLevel, nextLevel, levelHighScores, onSelectLevel, onBack) {
  ...
  draw(ctx, screenW, screenH) {
    ...
    ctx.fillText('SELECT LEVEL', screenW / 2, 50);

    ctx.font = '12px monospace';
    ctx.fillStyle = 'rgba(170,170,200,0.55)';
    ctx.fillText('Cleared levels can be replayed', screenW / 2, 70);

    const totalLevels = this._nextLevel; // show 1..nextLevel
    ...
    for (let i = 0; i < totalLevels; i++) {
      ...
      const isNext = level === this._nextLevel;
      const accent = isNext ? '#69f0ae' : '#90caf9';
      const best = this._levelHighScores[level];
